...
namespace ConsoleApp1
{
    ...
    public class Order:OrderDetails
    {
        ...
        public Order(string ID,string Name,string Orders,string Tel)
        {
            this.ID = ID;
            this.Name = Name;
            this.Orders = Orders;
        ...
        public static string[] list3 = new string[] { "13282737833", "15890782803", "13929492651", "15278786787" };
        public static string MySort(string m)    //查找
        {
            for (int i = 0; i <= 3; i++)
            {
                myList.Add($"2018111210{i} {List2[i]} {List1[i]} {list3[i]}");
            }
            for (int i = 0; i <= 3; i++)
            {
    ...
    class ConvertHtml
    {
        public static void ConvertXML()
        {
            try
            {
                ...
                doc.Load(@"OrderLists.xml");

                XPathNavigator nav = doc.CreateNavigator();
                nav.MoveToRoot();

                XslCompiledTransform xt = new XslCompiledTransform();
                xt.Load(@"..\..\myConvert.xslt");

                FileStream outFileStream = File.OpenWrite(@"..\..\Orderlists.html");
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            ConvertHtml.ConvertXML();                //将xml转换成html
            //反序列化
